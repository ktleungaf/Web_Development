...
using SinExWebApp20273938.ViewModels;
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (roleResult.Succeeded)
                    {
                        ...
                        if (model.PersonalInformation != null)
                        {
                            /*Requirement 1: A shipping account should be assigned a unique 12-digit account number by the system upon successful creation. */
                            model.PersonalInformation.AccountNumber = (Convert.ToString((1 + db.ShippingAccounts.Count()))).PadLeft(12, '0');
                            model.PersonalInformation.UserName = user.UserName;
                            db.ShippingAccounts.Add(model.PersonalInformation);
                            //db.SaveChanges();    Q: Should it be included?
                        }
                        else
                        {
                            /*Requirement 1: A shipping account should be assigned a unique 12-digit account number by the system upon successful creation. */
                            model.BusinessInformation.AccountNumber = (Convert.ToString((1 + db.ShippingAccounts.Count()))).PadLeft(12, '0');
                    ...
                    {
                        /*update ViewBag value
                        if (model.LoginInformation.Email == model.PersonalInformation.EmailAddress) {
                            ViewBag.AccountType = "Personal";
                        }
                        else if (model.LoginInformation.Email == model.BusinessInformation.EmailAddress) {
